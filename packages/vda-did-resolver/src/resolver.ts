...
export class VdaDidResolver {
  ...
  async getOwner(
    ...
    const result = await this.contracts[networkId].functions.identityOwner(
      address,
      { blockTag }
    );
    return result["0"];
  ...
  async getBlockMetadata(
    blockHeight: number,
    networkId: string
  ): Promise<{ height: string; isoDate: string }> {
    const block: Block = await this.contracts[networkId].provider.getBlock(
      blockHeight
    );
    return {
      height: block.number.toString(),
      isoDate: new Date(block.timestamp * 1000)
        .toISOString()
        .replace(".000", ""),
    };
  }
  ...
  async changeLog(
    ...
    while (previousChange) {
      ...
      const fromBlock =
        previousChange.toHexString() !== "0x00"
          ? previousChange.sub(1).toHexString()
          : previousChange.toHexString();
  ...
  wrapDidDocument(
    ...
    for (const event of history) {
      ...
      const eventIndex = `${event._eventName}-${
        (<DIDDelegateChanged>event).delegateType ||
        (<DIDAttributeChanged>event).name
      }-${
        (<DIDDelegateChanged>event).delegate ||
        (<DIDAttributeChanged>event).value
      }`;
      if (validTo && validTo.gte(now)) {
        if (event._eventName === eventNames.DIDDelegateChanged) {
          ...
          if (match) {
            ...
            const contextTag = Buffer.from("?context=", "utf-8").toString(
              "hex"
            );
    ...
    if (Object.values(keyAgreementRefs).length > 0) {
      didDocument.keyAgreement = keyAgreement.concat(
        Object.values(keyAgreementRefs)
      );
    }
    didDocument.assertionMethod = [
      ...(didDocument.verificationMethod?.map((pk) => pk.id) || []),
    ];
  ...
  async resolve(
    ...
    const { address, history, controllerKey, chainId } = await this.changeLog(
      id,
      networkId,
      "latest"
    );
    try {
      const { didDocument, deactivated, versionId, nextVersionId } =
        this.wrapDidDocument(
          did,
          address,
          controllerKey,
          history,
