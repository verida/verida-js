...
const validateDidDocument = async (account: AutoAccount, context: Context, did: string) => {
  ...
  assert.ok(
    fetchedStorageConfig.services.notificationServer,
    'Have a notification server in the DID document'
  );
  assert.equal(
    fetchedStorageConfig.services.notificationServer.type,
    ENDPOINT_CONFIG.defaultNotificationServer!.type,
    'Have correct notification server type'
  );
  assert.equal(
    fetchedStorageConfig.services.notificationServer.endpointUri,
    ENDPOINT_CONFIG.defaultNotificationServer!.endpointUri,
    'Have correct notification server endpointUri'
  );
};
...
describe('Verida notification tests', () => {
  ...
  describe('Sending messages', function () {
    ...
    it('can force add a notification server to an existing account context', async () => {
      ...
      assert.ok(
        !accountStorageConfig.services.notificationServer,
        `Don't have notification service endpoint in DID document`
      );
      ...
      const success1 = await account.linkStorageContextService(
        CONTEXT_1,
        DIDDocumentInterfaces.EndpointType.NOTIFICATION,
        'VeridaNotification',
        ENDPOINT_CONFIG.defaultNotificationServer.endpointUri
      );
