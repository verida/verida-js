...
export class VdaDID {
  ...
  async lookupOwner(cache = true): Promise<string> {
    return new Promise((resolve, reject) => {
      if (typeof this.controller === "undefined") {
        reject(
          "a web3 provider configuration is needed for network operations"
        );
      }
      if (cache && this.owner) {
        resolve(this.owner);
  ...
  async revokeDelegate(
    ...
    // const owner = await this.lookupOwner()
    return this.controller.revokeDelegate(
      delegateType,
      delegate,
      this.signKey,
      { ...txOptions /*, from: owner*/ }
  ...
  async setAttribute(
    ...
    return this.controller.setAttribute(
      key,
  ...
  async revokeAttribute(
    ...
    // const owner = await this.lookupOwner()
    return this.controller.revokeAttribute(
      key,
      attributeToHex(key, value),
      this.signKey,
      {
  ...
  async bulkAdd(
    ...
    // const owner = await this.lookupOwner()
    return this.controller.bulkAdd(
      controllerDParams,
      controllerAParams,
      this.signKey,
      {
  ...
  async bulkRevoke(
    ...
    // const owner = await this.lookupOwner()
    return this.controller.bulkRevoke(
      controllerDParams,
      controllerAParams,
      this.signKey,
      {
