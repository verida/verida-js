...
export default class DIDDocument {
  ...
  public async addContext(contextName: string, keyring: Keyring, endpoints: Endpoints) {
    ...
    // Add services
    this.addContextService(
      contextHash,
      EndpointType.DATABASE,
      endpoints.database.type,
      endpoints.database.endpointUri
    );
    this.addContextService(
      contextHash,
      EndpointType.MESSAGING,
      endpoints.messaging.type,
      endpoints.messaging.endpointUri
    );
    ...
    if (endpoints.storage) {
      this.addContextService(
        ...
        endpoints.storage.type,
        endpoints.storage.endpointUri
      );
    }
    ...
    if (endpoints.notification) {
      this.addContextService(
        ...
        endpoints.notification.type,
        endpoints.notification.endpointUri
      );
    }
  ...
  public removeContext(contextName: string): boolean {
    ...
    if (
      !this.doc.verificationMethod!.find((entry: VerificationMethod) => entry.id.match(contextDid))
    ) {
    ...
    this.doc.verificationMethod = this.doc.verificationMethod!.filter(
      (entry: VerificationMethod) => {
        return !entry.id.match(contextDid);
      }
    );
    this.doc.assertionMethod = this.doc.assertionMethod!.filter(
  ...
  public verifySig(data: any, signature: string): boolean {
    const verificationMethod = this.doc.verificationMethod!.find(
      (entry) => entry.id == this.doc.id
    );
  ...
  public verifyContextSignature(
    ...
    const verificationMethod = this.doc.verificationMethod!.find(
      (entry) => entry.id == publicKeyLookup
    );
