...
export default class AutoAccount extends Account {
  ...
  constructor(accountConfig: AccountConfig, autoConfig: NodeAccountConfig) {
    ...
    const didServerUrl = autoConfig.didServerUrl
      ? autoConfig.didServerUrl
      : Config.environments[autoConfig.environment].didServerUrl;
   ...
   * @returns
   */
  public async linkStorageContextService(
    contextName: string,
    endpointType: DIDDocumentInterfaces.EndpointType,
    serverType: string,
    endpointUri: string
  ) {
    return await StorageLink.setContextService(
      this.didClient,
      contextName,
      endpointType,
      serverType,
